...
class EKF:
    ...
    def reduce_mixture(self, ekfstate_mixture: MixtureParameters[GaussParams]) -> GaussParams:
        weights = ekfstate_mixture.weights
        means = np.array([comp.mean for comp in ekfstate_mixture.components])
        covariances = np.array([comp.cov for comp in ekfstate_mixture.components])
        return mixturereduction.gaussian_mixture_moments(weights ,means, covariances)
    ...
    def update(self,
        ...
        # TODO: the kalman gain, Hint: la.solve, la.inv.
        W = P@H.T@la.inv(S)
